...
- **macOS**: right-click the app, choose `Open`, then confirm (older macOS); or
  ...
  `xattr -dr com.apple.quarantine /path/to/ccscience.app`, then open again.
...
For third-party profiles, ccscience treats the active CC.Switch profile as
...
provider entry written by the live CC Switch app, ccscience follows the
...
requests through ccscience's hidden localhost forwarder, which normalizes
...
endpoint; ccscience normalizes the upstream URL so the path is not doubled.
For MiniMax, explicit profile `ANTHROPIC_BASE_URL` / `OPENAI_BASE_URL` wins; if
the profile only identifies MiniMax and provides a key, ccscience defaults
...
`ccscience status` to see the resolved provider source, base URL, provider
...
4. Installs user-level autostart:
   - macOS: `~/Library/LaunchAgents/io.github.ccscience.helper.plist`
...
open Claude Science from `ccscience`.
